...
# Load the data
file_path = "data/ampliconstats_freads.csv"
...
df_grouped['Method_Depth'] = df_grouped['Method'] + "_Depth_" + df_grouped['Depth'].astype(str)
...
sns.heatmap(heatmap_data_capped, cmap="viridis", norm=norm, cbar_kws={'label': 'FREADS (capped at 15000)'})
plt.title("Mean Amplicon Read Counts by Method and Depth (enhanced resolution 100–500)")
plt.xlabel("Amplicons")
plt.ylabel("Method / Depth")
plt.tight_layout()
plt.savefig("amplicon_heatmap_mean_capped_resolved.png", dpi=300)
plt.close()
